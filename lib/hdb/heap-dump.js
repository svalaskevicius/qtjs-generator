...
HeapDump.prototype.load = function (contents)
{
	...
	this.hd_fields = contents['snapshot']['meta']['node_fields'];
	this.hd_field_types = contents['snapshot']['meta']['node_types'];
	this.hd_type_types = this.hd_field_types[0]; /* XXX */
	this.hd_strings = contents['strings'];
	this.hd_nodes = contents['nodes'];
	this.hd_edges = contents['edges'];
	this.hd_nodeidx = 0;
	this.hd_edgeidx = 0;
	this.hd_graph = {};
	this.hd_nnodes = 0;
...
HeapDump.prototype.readNode = function ()
{
  ...
  node['children'] = [];
    ...
    var type = this.hd_edges[this.hd_edgeidx++];
    var name_or_index = this.hd_edges[this.hd_edgeidx++];
    var to_node = this.hd_edges[this.hd_edgeidx++];
		node['children'].push({'type':type, 'name_or_index': name_or_index, 'to_node': to_node});
	}
...
HeapDump.prototype.dbgexplore = function (out)
{
	...
	heap.computeDepths(this.hd_graph[0], 0);
