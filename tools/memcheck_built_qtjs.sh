...
valgrind --tool=memcheck --track-origins=yes --leak-check=full --show-reachable=yes --suppressions="$MYDIR"/.valgrind.suppress --gen-suppressions=all --log-file="$TOPDIR/build/memcheck.log" --xml=yes --xml-file="$TOPDIR/build/memcheck.xml" "$TOPDIR/build/runner/qtjs" "$@"
