...
#ifndef CPGF_META_QTCORE_QPROPERTYANIMATION_H
#define CPGF_META_QTCORE_QPROPERTYANIMATION_H
...
template <typename D>
void buildMetaClass_QPropertyAnimation(D _d)
{
    (void)_d;
...
public:
    ...
    QVariant interpolated(const QVariant & from, const QVariant & to, qreal progress) const
    {
        ...
        if(func)
        {
            return cpgf::fromVariant<QVariant >(cpgf::invokeScriptFunction(func.get(), this, from, to, progress).getValue());
        }
...
template <typename D>
void buildMetaClass_QPropertyAnimationWrapper(D _d)
{
    (void)_d;
    using namespace cpgf;
    
    
    QPropertyAnimationWrapper::cpgf__register(_d);
    
    buildMetaClass_QPropertyAnimation<D>(_d);
}
