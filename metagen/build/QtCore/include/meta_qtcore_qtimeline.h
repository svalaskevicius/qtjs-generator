...
#ifndef __META_QTCORE_QTIMELINE_H
#define __META_QTCORE_QTIMELINE_H
...
template <typename D>
void buildMetaClass_QTimeLine(const cpgf::GMetaDataConfigFlags & config, D _d)
{
...
public:
    ...
    qreal valueForTime(int msec) const
    {
        ...
        if(func)
        {
            return cpgf::fromVariant<qreal >(cpgf::invokeScriptFunction(func.get(), this, msec));
        }
...
template <typename D>
void buildMetaClass_QTimeLineWrapper(const cpgf::GMetaDataConfigFlags & config, D _d)
{
    (void)config; (void)_d; (void)_d;
    using namespace cpgf;
    
    
    QTimeLineWrapper::cpgf__register(config, _d);
    
    buildMetaClass_QTimeLine<D>(config, _d);
}
