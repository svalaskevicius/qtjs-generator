...
#ifndef __META_QTGUI_QMOVIE_H
#define __META_QTGUI_QMOVIE_H
...
template <typename D>
void buildMetaClass_QMovie(const cpgf::GMetaDataConfigFlags & config, D _d)
{
...
template <typename D>
void buildMetaClass_QMovieWrapper(const cpgf::GMetaDataConfigFlags & config, D _d)
{
    (void)config; (void)_d; (void)_d;
    using namespace cpgf;
    
    
    QMovieWrapper::cpgf__register(config, _d);
    
    buildMetaClass_QMovie<D>(config, _d);
}
