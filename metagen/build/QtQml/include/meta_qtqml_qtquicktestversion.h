// Auto generated file, don't modify.

#ifndef __META_QTQML_QTQUICKTESTVERSION_H
#define __META_QTQML_QTQUICKTESTVERSION_H


#include <qtQml_cpgf_compat.h>

#include "cpgf/gmetadefine.h"
#include "cpgf/metadata/gmetadataconfig.h"
#include "cpgf/metadata/private/gmetadata_header.h"
#include "cpgf/gmetapolicy.h"




namespace meta_qtqml { 


template <typename D>
void buildMetaClass_Global_qtquicktestversion(const cpgf::GMetaDataConfigFlags & config, D _d)
{
    (void)config; (void)_d; (void)_d;
    using namespace cpgf;
    
    _d.CPGF_MD_TEMPLATE _enum<long long>("GlobalDefine_QtQml_24")
        ._element("QTQUICKTEST_VERSION_STR", QTQUICKTEST_VERSION_STR)
        ._element("QTQUICKTEST_VERSION", QTQUICKTEST_VERSION)
    ;
}


} // namespace meta_qtqml




#include "cpgf/metadata/private/gmetadata_footer.h"


#endif
