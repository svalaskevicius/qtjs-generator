// Auto generated file, don't modify.

#ifndef __META_QTQML_QTQMLVERSION_H
#define __META_QTQML_QTQMLVERSION_H


#include <qtQml_cpgf_compat.h>

#include "cpgf/gmetadefine.h"
#include "cpgf/metadata/gmetadataconfig.h"
#include "cpgf/metadata/private/gmetadata_header.h"
#include "cpgf/gmetapolicy.h"




namespace meta_qtqml { 


template <typename D>
void buildMetaClass_Global_qtqmlversion(const cpgf::GMetaDataConfigFlags & config, D _d)
{
    (void)config; (void)_d; (void)_d;
    using namespace cpgf;
    
    _d.CPGF_MD_TEMPLATE _enum<long long>("GlobalDefine_QtQml_21")
        ._element("QTQML_VERSION_STR", QTQML_VERSION_STR)
        ._element("QTQML_VERSION", QTQML_VERSION)
    ;
}


} // namespace meta_qtqml




#include "cpgf/metadata/private/gmetadata_footer.h"


#endif
