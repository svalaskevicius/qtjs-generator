...
#ifndef __META_QTWIDGETS_QGRAPHICSVIEW_H
...
template <typename D>
void buildMetaClass_QGraphicsView(const cpgf::GMetaDataConfigFlags & config, D _d)
{
...
public:
    ...
    void drawItems(QPainter * painter, int numItems, QGraphicsItem ** items, const QStyleOptionGraphicsItem* options)
    {
        ...
        if(func)
        {
            cpgf::invokeScriptFunction(func.get(), this, painter, numItems, items, options);
            return;
        }
...
template <typename D>
void buildMetaClass_QGraphicsViewWrapper(const cpgf::GMetaDataConfigFlags & config, D _d)
{
    (void)config; (void)_d; (void)_d;
    using namespace cpgf;
    
    
    QGraphicsViewWrapper::cpgf__register(config, _d);
    
    buildMetaClass_QGraphicsView<D>(config, _d);
}
